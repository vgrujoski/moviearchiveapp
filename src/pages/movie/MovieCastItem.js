        ...
        { ActorsData.filter(actor => props.actorId === actor.id).map(
            filteredActor => (
                <Link to={`/actor/${filteredActor.slug}#top`} key={filteredActor.id}>
                    <div className="text-center h-full font-bold rounded-2xl overflow-hidden shadow-lg hover:bg-stone-200 dark:bg-black dark:bg-opacity-40 dark:hover:bg-black transition duration-300">
